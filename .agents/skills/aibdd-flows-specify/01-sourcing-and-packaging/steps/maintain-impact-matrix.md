...
2. wrapper 使用規範
   1. 固定流程：`init`（本輪 matrix 尚不存在時）→ 對 scope 內每一 impacted 檔各跑一次 `upsert` →（如有）對不再屬於本輪 scope 的既有 entry 各跑一次 `delete` → 最後一次 `validate`。
   ...
   6. 完成後 SNAPSHOT `$ENTRIES_AFTER`＝`list` 讀出的全量 entries（原樣 EMIT 給用戶）；NEW 路徑之 `$ENTRIES_BEFORE` 視為**空集合**。

3. 本步 scope：哪些檔要寫 entry
   1. 依上層 SOP 步驟 1.3 掃描結果，凡本輪 discovery 會 impact、對照、或條件式可能改動的**既有** boundary truth 規格檔，各寫一筆 entry。典型包含 `${CONTRACTS_DIR}/**`、`${DATA_DIR}/**`、`${TRUTH_BOUNDARY_PACKAGES_DIR}/**` 下**已存在**之 `.feature`、`.activity`、`dsl.yml` 等（掃描既有檔即可判定，**無需**先做 Action 萃取）。
   2. **既有** `${FEATURE_SPECS_DIR}` 下、本輪會 READ／對照或更新的 `.feature` 各寫一筆 entry（`read_only_compare`／`update`／`conditional_update`）。
   3. **本步不 forward-declare 本輪新增的 `.feature`／`.activity`（`add`）**：新 feature／activity 的 explicit 路徑必須由其萃取階段（Phase 02／03）決定，並由該階段 writeback 回 matrix。本步**不得**在 Action 萃取前先猜檔名／路徑或預先寫入其 `add` entry。
   4. 淘汰既存規格檔（`remove`）：依上層 SOP 步驟 1.3 掃描素材與 1.2 需求敘事，判定本輪需求**明文淘汰**的既存 `.feature`／`.activity`（Phase 02／03 之檔案 DELETE 即以這些 `remove` entries 為依據），各以 `change_type=remove` 寫一筆 entry——刪除也是本 plan 對該檔的影響，必須在 matrix 留下機讀紀錄；檔案刪除不在本步。
   5. 撤回情境（`delete`）：本輪稍後的修正（findings 就地修正、澄清結論、scope 縮小、誤判路徑改綁）使**先前已 upsert 的 entry 不再屬於本輪 impact** 時，對該 entry 跑一次 `delete`。`delete` 僅及 matrix entry，**不**刪任何規格檔案。
   6. 不寫入 entry 的範圍：plan-side artifacts（`${PLAN_REPORTS_DIR}/discovery-sourcing.md`、`${PLAN_SPEC}`、`${IMPACT_MATRIX_YML}` 本身）、`${TRUTH_BOUNDARY_ROOT}` 外的路徑、以及上層 SOP 步驟 1.4 尚未存在且本輪不打算新建內容的 placeholder 路徑。
...
4. `change_type` 怎麼選
   ...
   5. `remove`：檔在 plan 前已存在；本輪需求明文淘汰該既存規格檔。`impact_summary` 寫「淘汰什麼、依據哪段需求原文」。
