...
0. 在 CWD 底下 grep 搜尋 **/arguments.yml 檔案，如不存在檔案請直接 STOP，向使用者回報：「我在 CWD 底下找不到 **/arguments.yml 檔案，你是否已經執行過 /aibdd-kickoff 了？」

  0.1 RESOLVE arguments——將本 SOP 引用的變數透過 resolver 綁定，並把 resolver stdout（每行一筆 KEY=value）原樣顯示給用戶。Resolver 非 0 退出時，STOP 並把 stderr 透傳給用戶。
