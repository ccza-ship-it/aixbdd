...
6. 推論並收斂 state schema
   ...
   6.1 參考 `$DISCOVERY_TRUTH` 依 `$WORKLIST_QUOTES` 從資料流動 REASONING 建立資料狀態聚合分析、把資料客觀拆分成 Domain Aggregate／Entity／Value-Object（不腦補），作為 `$STATE_TARGETS`，每筆為 `{ target_path, scope, impact_id }`；`target_path` 為相對 `${DATA_DIR}` 的 flat 檔案路徑（不得含 `<<NN-functional-module>>` 借位子層，見 `aibdd-core::references/ssot/spec-package-paths.md`），`impact_id` 為驅動該 schema 的 `$WORKLIST` impact；本步只推理不落地。
