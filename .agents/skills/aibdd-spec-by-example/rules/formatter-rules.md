...
- 引號與 `And` 對齊：見 `aibdd-spec-by-example/rules/cucumber-literal-format.md` 檢核
...
→ `中置（條件）` 不在 4-pattern 對應表，pattern 無法分類；必須回分類步驟 DELEGATE 補上合法前綴。
...
- 標題滲入技術 ID：`Example: user-001 結帳失敗` ← 業務語言違規（識別維度）；若保留 ID 須在草擬時寫 `"user-001"`
