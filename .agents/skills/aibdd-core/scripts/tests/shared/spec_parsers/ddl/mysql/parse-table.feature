Feature: MySQLSpecParser collects one table Part per CREATE TABLE statement
  ...
  Rule: 後置（狀態）- 每張 CREATE TABLE 應產出單一 table Part
    Example: domain.mysql.sql 內兩張 table → 兩個 part
  ...
  Rule: 後置（狀態）- table Part 的 table_name 與 target_part_path 應對應原文
    Example: users / room_members table 各自具備正確 identity
      When MySQLSpecParser parses the last file
      Then the part named "users" has target_part_path "data/domain.mysql.sql#users"
      And the part named "room_members" has target_part_path "data/domain.mysql.sql#room_members"
  ...
  Rule: 後置（狀態）- 每張 table 之 columns 應包含全部欄位與其旗標
    Example: users 表 5 欄位、id 為 pk、created_at 帶 default
      ...
      Then the part named "users" has columns:
        ...
        | created_at | DATETIME | true     | false | true        |
