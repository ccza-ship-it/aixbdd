Feature: PostgresSpecParser derives not_null_columns from each CREATE TABLE
  ...
  Rule: 後置（狀態）- 每欄 target_part_path 應為 `<spec>#<table>.<column>`
    Example: room_members.room_no 與 room_members.player_id 路徑正確
      When PostgresSpecParser parses the last file
      Then the column "room_members.room_no" has target_part_path "data/domain.pg.sql#room_members.room_no"
      And the column "room_members.player_id" has target_part_path "data/domain.pg.sql#room_members.player_id"

  Rule: 後置（狀態）- PRIMARY KEY 欄位 nullable 應為 false
    Example: id SERIAL PRIMARY KEY → nullable false
      Given a temporary file at "data/domain.pg.sql" with content:
        """
        CREATE TABLE users (
